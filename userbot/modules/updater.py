...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        try:
            remote.push(refspec="HEAD:refs/heads/x-sql-extended", force=True) #pylint:disable=E0602
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("master", origin.refs.x-sql-extended) #pylint:disable=E0602
        repo.heads.master.set_tracking_branch(origin.refs.x-sql-extended) #pylint:disable=E0602
