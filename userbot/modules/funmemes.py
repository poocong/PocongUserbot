...
@bot.on(poci_cmd(outgoing=True, pattern=r"gangstar(?: |$)(.*)"))
async def _(event):
...
@bot.on(poci_cmd(outgoing=True, pattern=r"plane(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(poci_cmd(outgoing=True, pattern=r"dump(?: |$)(.*)"))
async def _(message):
    try:
...
@bot.on(poci_cmd(outgoing=True, pattern=r"kiss(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("😗😙😚😚😘"))
